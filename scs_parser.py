...
def evaluate_param(param,paramsd,evaluated_paramsd,parent=None,params_called_list=[]):
    """ Evaluates param value and puts it into dictionary for later use
        ...
        paramsd: dictionary of params which values may be yet to evaluated whith their expresions inside, param SHOUD be inside

        evaluated_paramsd: dictionary of already evaluated params, could be numerical value or symbols expresion (sympy)
    
        parent: parrent circuit of circuit which made the call, its for while we can't find definition in own dictionaries we can look for it in parents ones

        params_called_list: a list of params which we are already in process of evaluating so we don't call to evaluate them again, its for catching a circulary reference

        This funciotn expands the expresion for a parameter to its grammatic tokens and look for their value or calls to evaluate their values if they are on the list
        of paramters to be evaluated and weren't called for yet. When it completes to get all the needed values it calculates the expresion for that value, and
        puts it in the evaluated_paramsd. Raises an exception if definitions aren't found. Also raises exception if expresion is ill-formed.
    """
    def expand(tokens):
...
def evaluate_params(paramsd,parent=None):
    """ Evaluate parameters expresion to their values or symbols expresions
        ...
        Function takes the dictionary and try to evaluate each parameter one by one. Returns dictionary of evaluated parameters.
...
def evaluate_passed_params(paramsd,inst,evaluated_paramsd = {}):
...
def parse_param_expresion(expresion):
    """ Parses expresion for parameters
        ...
        Function parses the expresion. Expresion should be mathematical construct. Could contain variable names, numbers and operators, round brackets. 
        Function returns grammatical tokens for this expresion.
    """
    expresion.strip()
    tokens = []
    while expresion:
...
def results2values(tokens,instance):
    """ Translates tokens to their values and makes a single expresion out of them.
        ...
        instance: instance object which should hold value of parametrs or which could be called for function of it's solution (like v() etc.)

        It translate tokens list into one expresion. Single token could also be a list in which way it would be translated in whole expresion inside the bracket.        
    """
    ret_str = ''
    for token in tokens:
        if isinstance(token,list):                
...
def strip_comment(str):
    ...
    m = reg_comment.search(str)
    return m.group('not_comment') if m else str
    
def get_unnamed_params(str):
    ...
    while str:
        str.strip()
        m = reg_unnamed_param_with_rest.search(str)
        if m:
            str = m.group('rest').strip()  
            m2 = reg_unnamed_param.search(m.group('param'))
            param_l.append(m2.group('value'))
            continue
        m = reg_simple_param.search(str)
        if m:
            str = m.group('rest').strip() 
...
def get_params(str):
    """ Get dictionary of parameters from the end of string.
        ...
        Get dictionary of params from end of string and modyfies str in place to just hold rest of the string not being named parameters
        So string like .foo bar1 bar2 bar3 = spam1 bar4 = 'spam2'
        would return {bar3: spam1, bar4: spam2} and str would be: .foo bar1 bar2
    """
    str = str.strip()
    param_d = {}
...
def add_element(param_d,param_l,name,circuit):
    """ Adds element to circuit
        ...
        Function is on the list of function for getNameFunctionFromHead. It adds a element to circuit and returns this circuit.
    """
...
def include_file(param_d,param_l,name,circuit):
    """ Includes contents of another file into currentling parsing file.
        ...
        Function is on the list of function for getNameFunctionFromHead. It includes contents of another file into parsing process.
        It just start parsing this file. Returns circuit (could be modified while parsing include file)
    """
    parseFile(param_l[0],circuit)
    return circuit
...
def add_param(param_d,param_l,name,circuit):
    """ Adds parameter defintion to circuit
        ...
        Function is on the list of function for getNameFunctionFromHead. It adds a parameter definition to circuit and returns this circuit.
        Should be either one element dictionary or one element parameter list, else it doesn't add any defition
    """
...
def add_analysis(param_d,param_l,name,circuit):
    """ Adds element to top circuit
        ...
        Function is on the list of function for getNameFunctionFromHead. It adds an analysis to circuit and returns this circuit.
    """
...
def change_to_parent_circuit(param_d,param_l,name,circuit):
    """ Change working circuit to parent circuit
        ...
        Function is on the list of function for getNameFunctionFromHead. When .ends will be encountered in file this function will be executed.
        It will change the working circuit while parsing to parent of current circuit.
...
def get_name_function_from_head(head):
    """ Return appropriate function depending on head string
        ...
        head: string with header of a line, depending on it appropriate funcition to deal with rest of the line will be returned

        Depending if a line which header is provided as input is a control sentence (.foo) or element defintion (Xname etc.)
        will return appriopriate function to deal with rest of the line.
    """
    ...
    elif head[0] in ('R','r','L','l','C','c','V','v','I','i','E','e','H','h','G','g','F','f','X','x'):
...
def parseFile(filename,circuit):
...
__all__ = [add_analysis, add_element, add_param, add_subcircuit, change_to_parent_circuit, 
           ...
           parse_param_expresion, parseFile, parseLine, strip_comment]
